...
class DBAccess
{
    ...
    {
        // sql request
        $sql = "SELECT users.id as userID, surname,firstname,pseudo,phoneNumber,email,password,admin,activated, teams.id as teamID FROM users LEFT JOIN teams ON users.id = teams.id_users WHERE users.id = $id";
        // Execute request
        return $this->executeSqlRequest($sql);
    }
